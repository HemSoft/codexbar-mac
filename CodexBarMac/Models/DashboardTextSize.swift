...
private struct DashboardZoomActionsKey: FocusedValueKey {
    typealias Value = DashboardZoomActions
}

extension FocusedValues {
    var dashboardZoomActions: DashboardZoomActions? {
        get { self[DashboardZoomActionsKey.self] }
        set { self[DashboardZoomActionsKey.self] = newValue }
    }
}

struct DashboardZoomCommands: Commands {
    @FocusedValue(\.dashboardZoomActions) private var actions
    ...
    var body: some Commands {
        CommandGroup(after: .textEditing) {
            ...
            .disabled(actions?.canIncrease != true)

            Button("Zoom Out") {
                actions?.decrease()
            }
            .keyboardShortcut("-", modifiers: .command)
            .disabled(actions?.canDecrease != true)
